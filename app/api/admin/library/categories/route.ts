...
export async function GET() {
  ...
  const rows = await query<{
    ...
    `SELECT c.id, c.name, c.slug, c.color, c.icon, c.display_order, c.is_active,
            (SELECT COUNT(*)::int FROM books b WHERE b.category_id = c.id) AS books_count
       FROM categories c
      WHERE c.parent_id = $1
  ...
  try {
    ...
    const row = await queryOne<{ id: string; name: string; slug: string }>(
      `INSERT INTO categories
         (name, slug, parent_id, color, icon, display_order, is_active, created_at, updated_at, created_by)

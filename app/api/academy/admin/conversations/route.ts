...
export async function GET(req: NextRequest) {
    ...
    const allowedRoles: ("admin" | "supervisor")[] = ["admin", "supervisor"]
    ...
    const searchCondition = search
        ? `WHERE s.name ILIKE $1 OR r.name ILIKE $1 OR p.name ILIKE $1`
        : ''
    ...
    SELECT
      ...
      c.unread_count_student, c.unread_count_teacher, c.unread_count_parent, c.unread_count_admin, c.is_active,
      c.created_at, c.is_ticket, c.ticket_status, c.assigned_supervisor_id,
      s.id AS student_id, s.name AS student_name, s.email AS student_email,
      r.id AS teacher_id, r.name AS teacher_name, r.email AS teacher_email,

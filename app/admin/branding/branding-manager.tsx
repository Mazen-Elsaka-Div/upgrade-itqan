...
export function BrandingManager() {
  ...
  const LOGO_FIELDS: { key: keyof Branding; label: string; desc: string }[] = [
    { key: "logoUrl", label: ab?.mainLogo ?? "Main Logo", desc: ab?.mainLogoDesc ?? "Appears on the public site and in emails" },
    { key: "dashboardLogoUrl", label: ab?.dashboardLogo ?? "Dashboard Logo", desc: ab?.dashboardLogoDesc ?? "Appears in user dashboards" },
    { key: "faviconUrl", label: ab?.favicon ?? "Favicon", desc: ab?.faviconDesc ?? "Appears in the browser tab" },
  ]
    ...
    <div className="p-4 sm:p-6 max-w-5xl mx-auto space-y-6" dir={isAr ? "rtl" : "ltr"}>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl sm:text-2xl font-bold text-foreground">{ab?.title ?? "Brand Identity and Contact Info"}</h1>
            <p className="text-sm text-muted-foreground text-pretty">
              {ab?.desc ?? "Control platform logos, favicon, contact info, and social media links."}
            </p>
          </div>
        </div>
        <Button onClick={handleSave} disabled={saving} className="gap-2">
          {saving ? <Loader2 className="h-4 w-4 animate-spin" /> : saved ? <CheckCircle className="h-4 w-4" /> : <Save className="h-4 w-4" />}
          {saved ? (ab?.saved ?? "Saved") : (ab?.saveChanges ?? "Save Changes")}
        </Button>
      </div>
      ...
      <section className="rounded-xl border border-border bg-card p-4 sm:p-5">
        <h2 className="text-base font-semibold text-card-foreground mb-4">{ab?.logosAndIcons ?? "Logos and Icons"}</h2>
            ...
            <div key={f.key} className="space-y-2">
              ...
              <div className="flex items-center justify-center h-24 rounded-lg border border-dashed border-border bg-muted/40 overflow-hidden">
                {branding[f.key] ? (
                  ...
                  <img src={branding[f.key] || "/placeholder.svg"} alt={f.label} className="max-h-full max-w-full object-contain" />
                ) : (
      ...
      {/* Contact info */}
      <section className="rounded-xl border border-border bg-card p-4 sm:p-5">
        <h2 className="text-base font-semibold text-card-foreground mb-4">{ab?.contactInfo ?? "Contact Information"}</h2>
        <div className="grid gap-4 sm:grid-cols-2">
          <div className="space-y-1.5">
            <Label className="flex items-center gap-1.5 text-sm font-medium">
              <Mail className="h-3.5 w-3.5" /> {ab?.email ?? "Email"}</Label>
            <Input value={contact.email} onChange={(e) => setContact((p) => ({ ...p, email: e.target.value }))} dir="ltr" />
          </div>
          <div className="space-y-1.5">
            <Label className="flex items-center gap-1.5 text-sm font-medium">
              <Phone className="h-3.5 w-3.5" /> {ab?.phone ?? "Phone Number"}</Label>
            <Input value={contact.phone} onChange={(e) => setContact((p) => ({ ...p, phone: e.target.value }))} dir="ltr" />
          </div>
          <div className="space-y-1.5 sm:col-span-2">
            <Label className="flex items-center gap-1.5 text-sm font-medium">
              <MapPin className="h-3.5 w-3.5" /> {ab?.address ?? "Address"}</Label>
      ...
      <section className="rounded-xl border border-border bg-card p-4 sm:p-5">
        <h2 className="flex items-center gap-2 text-base font-semibold text-card-foreground mb-4">
          <Share2 className="h-4 w-4 text-primary" /> {ab?.socialLinks ?? "Social Media Links"}</h2>
        <div className="grid gap-4 sm:grid-cols-2">

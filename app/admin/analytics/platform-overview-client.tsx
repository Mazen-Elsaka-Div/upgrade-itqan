...
export function PlatformOverviewClient() {
  ...
  if (!data) {
    return <p className="py-12 text-center text-sm text-muted-foreground">تعذر تحميل الإحصائيات.</p>
  }
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500" dir="rtl">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-black text-foreground">نظرة عامة على المنصة</h1>
          <p className="text-sm text-muted-foreground">إحصائيات شاملة للمدير العام تجمع بين الأكاديمية والمقرأة.</p>
        </div>
      </div>

      {/* Members */}
      <section className="space-y-3">
        <h2 className="text-sm font-black uppercase tracking-wide text-muted-foreground">الأعضاء</h2>
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
          <StatCard icon={Users} label="إجمالي الأعضاء" value={data.users.total} tone="bg-primary/10 text-primary" />
          <StatCard
            icon={UserCheck}
        ...
        <Card className="rounded-3xl border border-blue-500/30 bg-blue-500/5">
          ...
          <CardContent className="grid grid-cols-3 gap-4">
            ...
            <div className="text-center">
              ...
              <p className="text-xs text-muted-foreground">الالتحاقات</p>
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="rounded-3xl border border-emerald-500/30 bg-emerald-500/5">
          ...
          <CardContent className="grid grid-cols-3 gap-4">
            <div className="text-center">
              ...
              <p className="text-xs text-muted-foreground">التلاوات</p>
            </div>
            <div className="text-center">
            ...
            <div className="text-center">
              ...
              <p className="text-xs text-muted-foreground">مُراجعة (7 أيام)</p>
            </div>
          </CardContent>
        </Card>
      </section>
      ...
      {/* Role distribution */}
      <section className="space-y-3">
        <h2 className="text-sm font-black uppercase tracking-wide text-muted-foreground">توزيع الأدوار</h2>
              ...
              data.roleDistribution.map((r) => (
                <div key={r.role} className="space-y-1">
                  <div className="flex items-center justify-between text-sm">
                    <span className="font-bold text-foreground">{ROLE_LABELS[r.role] ?? r.role}</span>

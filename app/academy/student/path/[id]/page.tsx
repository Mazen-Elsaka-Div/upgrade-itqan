...
export default function StudentPathDetailPage() {
  ...
  const fetchPathDetails = async () => {
    try {
      ...
      if (res.ok) {
        ...
        toast.error((t.addedTranslations_2026?.['فشل في تحميل تفاصيل المسار'] || 'فشل في تحميل تفاصيل المسار'))
      }
    } catch (error) {
  ...
  const handleEnroll = async () => {
    ...
    try {
      ...
      const data = await res.json()
      if (res.ok) {
        toast.success((t.addedTranslations_2026?.['تم التسجيل في المسار بنجاح!'] || 'تم التسجيل في المسار بنجاح!'))
        fetchPathDetails()
      } else {
        toast.error(data.error || (t.addedTranslations_2026?.['فشل التسجيل في المسار'] || 'فشل التسجيل في المسار'))
      }
    } catch (error) {
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[500px] space-y-4">
        ...
        <p className="text-muted-foreground animate-pulse text-sm">{(t.addedTranslations_2026?.['جاري تحميل تفاصيل المسار الأكاديمي...'] || 'جاري تحميل تفاصيل المسار الأكاديمي...')}</p>
      </div>
    )
  }
  ...
  if (!path) {
    return (
      <div className="text-center py-16 space-y-4">
        <h2 className="text-2xl font-bold text-destructive">{(t.addedTranslations_2026?.['المسار غير موجود'] || 'المسار غير موجود')}</h2>
        <p className="text-muted-foreground">{(t.addedTranslations_2026?.['عذراً، لم نتمكن من العثور على المسار المطلوب.'] || 'عذراً، لم نتمكن من العثور على المسار المطلوب.')}</p>
        <Link href="/academy/student/path" className="inline-flex items-center gap-2 text-emerald-600 font-semibold hover:underline">
          ...
          {(t.addedTranslations_2026?.['العودة لقائمة المسارات'] || 'العودة لقائمة المسارات')}
                        </Link>
      </div>
    )
  }
  ...
  const levelLabels = {
    beginner: (t.addedTranslations_2026?.['مبتدئ'] || 'مبتدئ'),
    intermediate: (t.addedTranslations_2026?.['متوسط'] || 'متوسط'),
    advanced: (t.addedTranslations_2026?.['متقدم'] || 'متقدم')
  }
  ...
  const subjectLabels: { [key: string]: string } = {
    fiqh: (t.addedTranslations_2026?.['الفقه الإسلامي'] || 'الفقه الإسلامي'),
    aqeedah: (t.addedTranslations_2026?.['العقيدة الإسلامية'] || 'العقيدة الإسلامية'),
    seerah: (t.addedTranslations_2026?.['السيرة النبوية'] || 'السيرة النبوية'),
    tafsir: (t.addedTranslations_2026?.['التفسير وعلوم القرآن'] || 'التفسير وعلوم القرآن'),
    tajweed: (t.addedTranslations_2026?.['التجويد والمقرأة'] || 'التجويد والمقرأة')
  }

  const getCertLabel = (type?: string) => {
    if (!type) return (t.addedTranslations_2026?.['شهادة إنجاز للمسار'] || 'شهادة إنجاز للمسار')
    if (type === 'certificate_of_completion') return (t.addedTranslations_2026?.['شهادة إتمام معتمدة'] || 'شهادة إتمام معتمدة')
    if (type === 'certificate_of_attendance') return (t.addedTranslations_2026?.['شهادة حضور'] || 'شهادة حضور')
    return type
  }
    ...
    return (
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
        ...
        <Link href="/academy/student/path" className="inline-flex items-center gap-1.5 text-sm text-muted-foreground hover:text-emerald-600 transition-colors">
          <ArrowRight className="w-4 h-4" />
          {(t.addedTranslations_2026?.['العودة لقائمة المسارات الأكاديمية'] || 'العودة لقائمة المسارات الأكاديمية')}
                        </Link>
        ...
        <div className="grid lg:grid-cols-3 gap-8 items-start">
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            {path.promo_video_url && (
              <div className="bg-card border border-border/80 rounded-3xl p-6 shadow-sm space-y-4">
                <h2 className="text-xl font-bold flex items-center gap-2">
                  ...
                  {(t.addedTranslations_2026?.['الفيديو الترويجي للمسار'] || 'الفيديو الترويجي للمسار')}
                                                </h2>
                <div className="relative aspect-video rounded-2xl overflow-hidden bg-slate-900 border border-border/60 group shadow-inner">
                  {!isVideoPlaying ? (
                    <div className="absolute inset-0 flex flex-col items-center justify-center p-4">
                      ...
                      <p className="text-slate-300 text-sm mt-3 font-medium z-10">{(t.addedTranslations_2026?.['شاهد مقدمة تعريفية قصيرة بالمسار'] || 'شاهد مقدمة تعريفية قصيرة بالمسار')}</p>
            ...
            {whatYouWillLearn.length > 0 && (
              <div className="bg-card border border-border/80 rounded-3xl p-6 sm:p-8 shadow-sm space-y-5">
                <h2 className="text-xl font-bold flex items-center gap-2">
                  ...
                  {(t.addedTranslations_2026?.['ماذا ستتعلم في هذا المسار الأكاديمي؟'] || 'ماذا ستتعلم في هذا المسار الأكاديمي؟')}
                                                </h2>
              ...
              {path.target_audience && (
                <div className="bg-card border border-border/80 rounded-3xl p-6 shadow-sm space-y-3">
                  <h3 className="font-bold text-lg flex items-center gap-2">
                    ...
                    {(t.addedTranslations_2026?.['الفئة المستهدفة'] || 'الفئة المستهدفة')}
                                                      </h3>
              ...
              {prerequisites.length > 0 && (
                <div className="bg-card border border-border/80 rounded-3xl p-6 shadow-sm space-y-3">
                  <h3 className="font-bold text-lg flex items-center gap-2">
                    ...
                    {(t.addedTranslations_2026?.['المتطلبات المسبقة'] || 'المتطلبات المسبقة')}
                                                      </h3>
            ...
            {/* Curriculum Preview (محتوى المسار) */}
            {stages.length > 0 && (
              <div className="bg-card border border-border/80 rounded-3xl p-6 sm:p-8 shadow-sm space-y-6">
                <h2 className="text-xl font-bold flex items-center gap-2 border-b border-border/80 pb-4">
                  ...
                  {(t.addedTranslations_2026?.['محتوى ومراحل المسار'] || 'محتوى ومراحل المسار')}
                                                </h2>
                
                <div className="relative pr-6 border-r-2 border-border/80 space-y-6 mr-3">
                  {stages.map((stage, idx) => (
                    <div key={stage.id} className="relative">
                      {/* Timeline node icon */}
                      <div className="absolute -right-[35px] top-1.5 w-6 h-6 rounded-full border-4 border-card flex items-center justify-center z-10 shadow-sm bg-slate-200 dark:bg-slate-800 text-slate-400">
                        <span className="text-[10px] font-bold text-slate-500 dark:text-slate-400">{idx + 1}</span>
                      </div>
                      
                      {/* Stage card */}
                      <div className="rounded-2xl border border-border/60 bg-muted/20 p-5 sm:p-6 opacity-80 hover:opacity-100 transition-opacity">
                        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                          <div>
                            <span className="text-xs font-semibold text-muted-foreground/80">{(t.addedTranslations_2026?.['المرحلة'] || 'المرحلة')} {idx + 1}</span>
                          ...
                          <div className="shrink-0">
                            {stage.course_id && (
                              <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 text-xs font-bold border border-emerald-500/20">
                                <BookOpen className="w-3.5 h-3.5" /> {(t.addedTranslations_2026?.['دورة:'] || 'دورة:')} {stage.course_title}
                              </span>
                            )}
                            {stage.halaqa_id && (
                              <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-teal-500/10 text-teal-600 dark:text-teal-400 text-xs font-bold border border-teal-500/20">
                                <GraduationCap className="w-3.5 h-3.5" /> {(t.addedTranslations_2026?.['حلقة تطبيقية'] || 'حلقة تطبيقية')}
                                                                            </span>
                            )}
                            {!stage.course_id && !stage.halaqa_id && (
                              <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-amber-500/10 text-amber-600 dark:text-amber-400 text-xs font-bold border border-amber-500/20">
                                <BookMarked className="w-3.5 h-3.5" /> {(t.addedTranslations_2026?.['درس نظري'] || 'درس نظري')}
                                                                            </span>
                            )}
                          </div>
                        </div>
          ...
          <div className="lg:col-span-1 lg:sticky lg:top-8 bg-card border border-border rounded-3xl shadow-lg overflow-hidden">
            ...
            <div className="h-36 bg-gradient-to-tr from-emerald-800 to-teal-900 flex flex-col items-center justify-center p-6 text-white text-center">
              ...
              <span className="text-xs tracking-widest text-emerald-200 uppercase font-bold">{(t.addedTranslations_2026?.['شهادة معتمدة عند الإتمام'] || 'شهادة معتمدة عند الإتمام')}</span>
            ...
            <div className="p-6 space-y-6">
              <div className="space-y-2">
                <span className="text-xs text-muted-foreground uppercase font-semibold">{(t.addedTranslations_2026?.['قيمة الاستثمار في المسار'] || 'قيمة الاستثمار في المسار')}</span>
                <div className="flex items-baseline gap-2">
                  <span className="text-3xl font-extrabold text-foreground">
                    {path.enrollment_type === 'paid' ? `${path.price} ر.س` : (t.addedTranslations_2026?.['مجاني'] || 'مجاني')}
                  </span>
                  {path.enrollment_type === 'paid' && (
                    <span className="text-xs text-muted-foreground">{(t.addedTranslations_2026?.['رسوم تدفع لمرة واحدة'] || 'رسوم تدفع لمرة واحدة')}</span>
                  )}
                </div>
              </div>
              ...
              <div className="space-y-3 border-y border-border/60 py-4 text-sm">
                <div className="flex justify-between">
                  <span className="text-muted-foreground flex items-center gap-1.5">
                    ...
                    {(t.addedTranslations_2026?.['مجموع المراحل الدراسيّة'] || 'مجموع المراحل الدراسيّة')}
                                                      </span>
                  <span className="font-semibold text-foreground">{path.total_stages} {(t.addedTranslations_2026?.['دورات'] || 'دورات')}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-muted-foreground flex items-center gap-1.5">
                    <Clock className="w-4 h-4 text-emerald-600" />
                    {(t.addedTranslations_2026?.['الوقت المقدر للدراسة'] || 'الوقت المقدر للدراسة')}
                                                      </span>
                  <span className="font-semibold text-foreground">{path.estimated_days} {(t.addedTranslations_2026?.['ساعة'] || 'ساعة')}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-muted-foreground flex items-center gap-1.5">
                    <Award className="w-4 h-4 text-emerald-600" />
                    {(t.addedTranslations_2026?.['الشهادة المقدمة'] || 'الشهادة المقدمة')}
                                                      </span>
              ...
              >
                {enrolling ? (
                  <>
                    ...
                    {(t.addedTranslations_2026?.['جاري التسجيل...'] || 'جاري التسجيل...')}
                                                      </>
                ) : (
                  (t.addedTranslations_2026?.['سجل وابدأ دراسة المسار الآن'] || 'سجل وابدأ دراسة المسار الآن')
                )}
              </button>

              <p className="text-[10px] text-center text-muted-foreground/80 leading-relaxed">
                {(t.addedTranslations_2026?.['بالتسجيل في هذا المسار، ستحصل على وصول فوري لجميع الدورات المربوطة بمراحل المسار وتخضع لشروط تتبع التقدم والتقييم.'] || 'بالتسجيل في هذا المسار، ستحصل على وصول فوري لجميع الدورات المربوطة بمراحل المسار وتخضع لشروط تتبع التقدم والتقييم.')}
                                          </p>
            </div>
          </div>
        </div>
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
        ...
        <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-emerald-700 via-emerald-800 to-teal-900 text-white p-6 sm:p-8 shadow-xl border border-emerald-700/50">
          ...
          <div className="relative z-10 flex flex-col sm:flex-row sm:items-center justify-between gap-6">
            <div className="flex items-start gap-4">
              ...
              <div>
                <h2 className="text-xl sm:text-2xl font-extrabold leading-snug">{(t.addedTranslations_2026?.['مبارك! أكملت هذا المسار بالكامل'] || 'مبارك! أكملت هذا المسار بالكامل')}</h2>
                <p className="text-emerald-100/90 mt-1.5 text-sm leading-relaxed max-w-xl">
                  {(t.addedTranslations_2026?.['أتممت جميع مراحل المسار بنجاح. شهادتك أصبحت متاحة في مركز الشهادات، ويمكنك مراجعة المسار في أي وقت من الأرشيف.'] || 'أتممت جميع مراحل المسار بنجاح. شهادتك أصبحت متاحة في مركز الشهادات، ويمكنك مراجعة المسار في أي وقت من الأرشيف.')}
                                                  </p>
              </div>
            </div>
            <div className="flex flex-col sm:flex-row gap-3 shrink-0">
              ...
              >
                ...
                {(t.addedTranslations_2026?.['شهادتي'] || 'شهادتي')}
                                            </Link>
              <Link
              ...
              >
                ...
                {(t.addedTranslations_2026?.['أرشيف إنجازاتي'] || 'أرشيف إنجازاتي')}
                                            </Link>
            </div>
          </div>
        </div>
      ...
      <div className="bg-card border border-border rounded-3xl p-6 sm:p-8 shadow-sm flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        ...
        <div className="flex items-center gap-4 bg-muted/30 border border-border/40 p-4 rounded-2xl shrink-0 w-full md:w-auto">
          ...
          <div>
            <span className="text-[10px] text-muted-foreground uppercase font-bold">{(t.addedTranslations_2026?.['التقدم العام'] || 'التقدم العام')}</span>
            ...
            <p className="text-xs text-muted-foreground mt-1 font-medium">
              {(t.addedTranslations_2026?.['أكملت'] || 'أكملت')} {enrollment.stages_completed} {(t.addedTranslations_2026?.['من أصل'] || 'من أصل')} {path.total_stages} {(t.addedTranslations_2026?.['مرحلة'] || 'مرحلة')}
                                      </p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-card border border-border rounded-3xl p-6 sm:p-8 shadow-sm space-y-6">
        <h2 className="text-xl font-bold flex items-center gap-2 border-b border-border/80 pb-4">
          <BookOpen className="w-5 h-5 text-emerald-600" />
          {(t.addedTranslations_2026?.['منهج ومراحل المسار'] || 'منهج ومراحل المسار')}
                          </h2>
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <div>
                      ...
                      {!isStageLocked && (
                        ...
                        >
                          {isStageCompleted ? (t.addedTranslations_2026?.['مراجعة المرحلة'] || 'مراجعة المرحلة') : 'دخول المرحلة'}
                        ...
                        <div className="bg-gradient-to-br from-slate-50 to-slate-100 dark:from-slate-800/80 dark:to-slate-900/80 p-3 rounded-2xl border border-border/80 shadow-sm flex flex-col gap-3 group/course transition-colors hover:border-emerald-500/30">
                          <div className="flex gap-3">
                            ...
                            <div className="flex flex-col justify-center flex-1 min-w-0">
                              <span className="text-[10px] text-emerald-600 dark:text-emerald-400 font-bold uppercase tracking-wider mb-0.5">{(t.addedTranslations_2026?.['متطلب الدورة'] || 'متطلب الدورة')}</span>
                              <h4 className="text-sm font-semibold text-foreground truncate" title={stage.course_title || (t.addedTranslations_2026?.['دورة أكاديمية'] || 'دورة أكاديمية')}>
                                {stage.course_title || (t.addedTranslations_2026?.['دورة أكاديمية مرتبطة'] || 'دورة أكاديمية مرتبطة')}
                              </h4>
                            </div>
                          </div>
                          ...
                          <div className="space-y-1.5 px-1">
                            <div className="flex justify-between items-center text-[10px] font-semibold">
                              <span className="text-muted-foreground">{(t.addedTranslations_2026?.['نسبة إنجازك في الدورة'] || 'نسبة إنجازك في الدورة')}</span>
                              <span className={cn(
                                "transition-colors",
                      ...
                      <div className="flex flex-col gap-3 mt-4 sm:mt-0 sm:max-w-[320px] w-full shrink-0">
                        <div className="bg-gradient-to-br from-emerald-50 to-teal-100/50 dark:from-emerald-950/40 dark:to-teal-900/30 p-3.5 rounded-2xl border border-emerald-200/50 dark:border-emerald-800/50 shadow-sm hover:shadow-md transition-all duration-300 group/halaqa hover:border-emerald-400/50 flex flex-col gap-3 relative overflow-hidden">
                          ...
                          <div className="flex gap-3 relative z-10">
                            ...
                            <div className="flex flex-col justify-center flex-1 min-w-0">
                              <span className="text-[10px] text-emerald-700 dark:text-emerald-400 font-extrabold uppercase tracking-widest mb-0.5">{(t.addedTranslations_2026?.['حلقة تطبيقية'] || 'حلقة تطبيقية')}</span>
                      ...
                      <div className="flex items-center gap-2 shrink-0">
                        ...
                        >
                          {completingStageId === stage.id ? (
                            ...
                            (t.addedTranslations_2026?.['إكمال المرحلة فوراً'] || 'إكمال المرحلة فوراً')
                          )}
                        </button>
                      </div>
                    )}

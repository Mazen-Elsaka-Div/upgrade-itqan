...
export default function SeriesDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const handleRemoveItem = async (itemId: string) => {
    if (!confirm(a.removeItemConfirm)) return
  ...
  if (!series) {
    return (
      <div className="text-center py-20">
        <p className="text-muted-foreground">{a.seriesNotFound}</p>
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <div className="flex items-center gap-3 mt-2 text-sm text-muted-foreground">
            {series.teacher_name && <span>{a.seriesTeacherName} {series.teacher_name}</span>}
            <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${
              series.is_published
                ? 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400'
                : 'bg-yellow-100 text-yellow-700 dark:bg-yellow-900/30 dark:text-yellow-400'
            }`}>
              {series.is_published ? a.seriesPublished : a.courseStatus.draft}
            </span>
          </div>
        </div>
        <button
      ...
      {items.length === 0 ? (
        <div className="bg-card border border-border rounded-xl p-16 text-center">
          ...
          <p className="text-muted-foreground font-medium mb-2">{a.noSeriesItems}</p>
          <p className="text-sm text-muted-foreground mb-4">{a.addSeriesItemsDesc}</p>
          <button
            onClick={() => { setShowAddModal(true); setSelectedItemId(''); setAddType('course') }}
            className="px-6 py-2.5 bg-emerald-600 text-white rounded-lg font-bold hover:bg-emerald-700 transition-colors"
          >
            <Plus className="w-4 h-4 inline ml-1" /> {a.addFirstItem}
          </button>
        </div>
      ) : (
          ...
          {items.map((item, index) => (
            <div key={item.id} className="bg-card border border-border rounded-xl p-4 flex items-center gap-4 hover:shadow-sm transition-shadow">
              ...
              <div className="flex-1 min-w-0">
                <p className="font-bold truncate">{item.title || a.untitled}</p>
                <p className="text-xs text-muted-foreground mt-0.5">
                  {item.item_type === 'course' ? a.courseType : a.learningPathType}
                </p>
              </div>
              <button
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm" onClick={(e) => e.target === e.currentTarget && setShowAddModal(false)}>
          <div className="bg-card border border-border rounded-2xl shadow-2xl w-full max-w-lg">
            <div className="flex items-center justify-between p-6 border-b border-border">
              <h3 className="text-lg font-bold">{a.addItemToSeries}</h3>
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="flex gap-2 p-1 bg-muted rounded-lg">
                ...
                >
                  <BookOpen className="w-4 h-4" /> {a.courseTab}
                </button>
                <button
                ...
                >
                  <Route className="w-4 h-4" /> {a.learningPathTab}
                </button>
              </div>
              ...
              <div>
                <label className="text-sm font-bold block mb-1.5">
                  {a.chooseItem.replace('{type}', addType === 'course' ? a.courseType : a.learningPathType)}
                </label>
                <select
                ...
                >
                  <option value="">{a.choosePlaceholder}</option>
                  {addType === 'course'
                ...
                {addType === 'course' && availableCourses.length === 0 && (
                  <p className="text-xs text-muted-foreground mt-1">{a.noAvailableCourses}</p>
                )}
                {addType === 'path' && availablePaths.length === 0 && (
                  <p className="text-xs text-muted-foreground mt-1">{a.noAvailablePaths}</p>
                )}
              </div>

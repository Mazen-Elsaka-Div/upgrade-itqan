...
function SupervisionTab({ isAr, t }: { isAr: boolean, t: any }) {
    ...
    const openConvo = async (c: Conversation) => {
        ...
        try {
            const res = await fetch(`/api/academy/admin/conversations/${c.id}`)
            if (res.ok) {
                const data = await res.json()
    ...
    const toggleActive = async (id: string, is_active: boolean) => {
        await fetch('/api/academy/admin/conversations', {
            method: 'PATCH',
                        ...
                        {loadingMessages ? (
                            ...
                            messages.map(m => (
                                <div key={m.id} className={`flex gap-3 ${m.sender_role === 'teacher' ? '' : 'flex-row-reverse'}`}>
                                    <div className={`w-10 h-10 rounded-xl flex items-center justify-center text-sm font-black shrink-0 shadow-sm border ${m.sender_role === 'teacher' ? 'bg-card text-emerald-400 border-border' : 'bg-card text-blue-400 border-border'}`}>
                                        {m.sender_name?.[0] || '?'}
                                    </div>
                                    <div className={`max-w-[80%] rounded-2xl px-4 py-3 text-sm shadow-sm ${m.sender_role === 'teacher'
...
function ContactMessagesTab({ isAr, t }: { isAr: boolean, t: any }) {
    ...
    const fetchMessages = useCallback(async () => {
        setLoading(true)
        try {
            const res = await fetch('/api/academy/admin/contact-messages')
            if (res.ok) {
                const data = await res.json()
                setMessages(data.messages)
            }
...
function DirectChatTab({ isAr, t }: { isAr: boolean, t: any }) {
    ...
    const openConversation = useCallback(async (conv: Conversation) => {
        ...
        try {
            const res = await fetch(`/api/academy/conversations/${conv.id}/messages`)
                ...
                if (!didInit.current && initialUserId && initialUserRole) {
                    ...
                    const existing = convs.find(c =>
                        !c.is_ticket && (
                            ...
                            (initialUserRole === "teacher" && c.teacher_id === initialUserId)
                        )
                    )
                    if (existing) {
                        openConversation(existing)
                    } else {
                        const cr = await fetch("/api/academy/conversations", {
                            method: "POST",
                            headers: { "Content-Type": "application/json" },
                        ...
                        if (cr.ok) {
                            ...
                            const newConvRes = await fetch("/api/academy/conversations")
    ...
    const sendMessage = async () => {
        ...
        setText("")
        try {
            await fetch(`/api/academy/conversations/${activeConv.id}/messages`, {
                method: "POST",
                headers: { "Content-Type": "application/json" },
...
function TicketsTab({ isAr, t }: { isAr: boolean, t: any }) {
    ...
    const updateTicketStatus = async (id: string, status: string) => {
        try {
            const res = await fetch(`/api/academy/admin/conversations`, {
                method: "PATCH",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ id, ticket_status: status }),
            })
            if (res.ok) {
                const cRes = await fetch("/api/academy/conversations")
    ...
    useEffect(() => {
        ...
        fetch("/api/academy/admin/users?role=supervisors").then(r => r.json()).then(d => setSupervisors(d.users || []))
    }, [])

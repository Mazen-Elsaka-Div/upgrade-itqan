    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground mt-1">{a.total} {students.length} {t.admin.students}</p>
        </div>
      </div>
      ...
      <div className="grid sm:grid-cols-3 gap-4">
        <div className="bg-card border border-border rounded-xl p-4">
          <p className="text-sm text-muted-foreground">{a.totalStudents}</p>
          <p className="text-3xl font-bold text-blue-600 mt-1">{students.length}</p>
        </div>
        <div className="bg-card border border-border rounded-xl p-4">
          <p className="text-sm text-muted-foreground">{a.inActiveCourses}</p>
          <p className="text-3xl font-bold text-green-600 mt-1">
            {students.filter(s => s.active_courses > 0).length}
          </p>
        </div>
        <div className="bg-card border border-border rounded-xl p-4">
          <p className="text-sm text-muted-foreground">{a.totalEnrollments}</p>
        ...
        {filtered.length === 0 ? (
          <div className="p-12 text-center">
            ...
            <p className="text-muted-foreground">
              {searchQuery ? a.noSearchResults : a.noStudentsYet}
            </p>
          </div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead className="bg-muted/50 border-b border-border">
                <tr>
                  <th className="text-right p-4 text-sm font-bold text-muted-foreground">{a.student}</th>
                  <th className="text-right p-4 text-sm font-bold text-muted-foreground">{a.gender}</th>
                  <th className="text-right p-4 text-sm font-bold text-muted-foreground">{a.coursesActiveTotal}</th>
                  <th className="text-right p-4 text-sm font-bold text-muted-foreground">{a.joinDate}</th>
                </tr>
              </thead>
                  ...
                  <tr key={student.id} className="hover:bg-muted/30 transition-colors">
                    ...
                    <td className="p-4 text-sm">
                      <span className="px-2 py-1 bg-muted rounded-full text-xs font-medium">
                        {student.gender === 'male' ? a.male : student.gender === 'female' ? a.female : '—'}
                      </span>
                    </td>
                    <td className="p-4">

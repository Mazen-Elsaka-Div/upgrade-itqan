...
export default function EditBookPage() {
  ...
  useEffect(() => {
    ...
    const run = async () => {
      setLoading(true)
      try {
        const res = await fetch(`/api/academy/admin/library/books/${id}`)
  ...
  const uploadLanguageFile = async (file: File) => {
    ...
    try {
      ...
      const saveRes = await fetch(`/api/academy/admin/library/books/${id}/files`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
      ...
      setNewLanguageLabel("")
      // Reload list
      const fresh = await fetch(`/api/academy/admin/library/books/${id}`)
      if (fresh.ok) {
  ...
  const deleteFile = async (fileId: string) => {
    ...
    try {
      const res = await fetch(`/api/academy/admin/library/books/${id}/files/${fileId}`, {
        method: "DELETE",
      })
      if (!res.ok) throw new Error()

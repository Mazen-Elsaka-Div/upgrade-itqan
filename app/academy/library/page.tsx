...
export default function PublicLibraryPage() {
  ...
  return (
    <div className="min-h-screen bg-background/50 flex flex-col" dir="rtl">
      {/* Hero Section */}
        ...
        <div className="container mx-auto px-4 py-16 sm:py-24 relative z-10">
          <div className="max-w-3xl space-y-6">
            <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-primary/10 text-primary text-sm font-semibold border border-primary/20">
              ...
              <span>{isAr ? "مكتبة إتقان" : "Itqan Library"}</span>
            </div>
            ...
            <p className="text-lg sm:text-xl text-muted-foreground leading-relaxed max-w-2xl">
              {lib?.subtitle || "استكشف آلاف الكتب القيمة بلغات متعددة لتعزيز معرفتك وتطوير مهاراتك."}
            </p>

            {/* Main Search Bar in Hero */}
            <div className="mt-8 relative group max-w-xl">
              <div className="absolute inset-0 bg-primary/20 rounded-2xl blur-xl transition-all duration-300 group-hover:bg-primary/30 group-hover:blur-2xl opacity-50" />
              <div className="relative flex items-center bg-card/90 backdrop-blur-md border border-primary/20 rounded-2xl p-2 shadow-xl transition-all duration-300 hover:border-primary/40">
                <Search className="w-6 h-6 text-primary ml-3" />
                <Input
                  value={search}
                  onChange={(e) => setSearch(e.target.value)}
                  placeholder={lib?.searchPlaceholder || "ابحث بالعنوان، المؤلف، أو الموضوع..."}
                  className="flex-1 bg-transparent border-none shadow-none text-lg placeholder:text-muted-foreground/60 focus-visible:ring-0 px-2 h-12"
                />
                <div className="bg-primary/10 text-primary px-4 py-2 rounded-xl text-sm font-bold hidden sm:block">
      ...
      <div className="container mx-auto px-4 py-8 flex-1 flex flex-col">
        {/* Filters Section */}
        <div className="flex flex-col sm:flex-row gap-4 mb-8 bg-card/40 backdrop-blur-sm border border-border/50 p-4 rounded-2xl">
          <div className="flex items-center gap-2 text-foreground/80 px-2">
            <Filter className="w-5 h-5 text-primary" />
            <span className="font-bold text-sm">تصفية النتائج:</span>
          </div>
          
          <div className="relative flex-1 group">
            <LayoutGrid className="absolute right-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground pointer-events-none group-hover:text-primary transition-colors" />
            <select
              value={category}
              onChange={(e) => setCategory(e.target.value)}
              className="w-full appearance-none bg-background border border-border/50 rounded-xl pr-10 pl-4 h-11 text-sm font-medium hover:border-primary/40 focus:border-primary focus:ring-1 focus:ring-primary transition-all cursor-pointer outline-none"
            >
          ...
          <div className="relative flex-1 group">
            <Globe className="absolute right-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground pointer-events-none group-hover:text-primary transition-colors" />
            <select
              value={language}
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6 mb-12">
            {books.map((book) => (
              ...
              >
                <div className="h-full flex flex-col bg-card border border-border/50 rounded-2xl overflow-hidden hover:shadow-2xl hover:shadow-primary/10 hover:-translate-y-1 transition-all duration-300 group-focus-visible:ring-2 ring-primary ring-offset-2 ring-offset-background">
                  {/* Image Container */}
                  <div className="relative aspect-[3/4] overflow-hidden bg-gradient-to-br from-muted to-muted/50">
                    {book.cover_image_url ? (
                      ...
                      <div className="w-full h-full flex flex-col items-center justify-center bg-primary/5 text-primary/30 gap-3">
                        <BookOpen className="w-16 h-16" />
                        <span className="text-xs font-bold uppercase tracking-widest opacity-50">بدون غلاف</span>
                      </div>
                    )}
                    
                    {/* Hover Overlay */}
                    <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-end p-4">
                      <div className="translate-y-4 group-hover:translate-y-0 transition-transform duration-300 text-white font-medium text-sm flex items-center gap-2">
                    ...
                    {book.category_name && (
                      <div className="absolute top-3 right-3 bg-black/60 backdrop-blur-md text-white text-[10px] font-bold px-2.5 py-1 rounded-full shadow-lg border border-white/10">
                        {book.category_name}
                      </div>
                    )}
                  </div>

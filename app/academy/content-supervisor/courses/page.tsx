    ...
    <div className="space-y-8 animate-in fade-in duration-500 pb-8">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div className="space-y-1">
          ...
          <p className="text-muted-foreground text-sm flex items-center gap-2">
            ...
            مراجعة الدورات الكاملة المرسلة من المعلمين قبل نشرها
          </p>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="bg-card border border-border/50 rounded-2xl p-16 text-center flex flex-col items-center justify-center shadow-sm">
          ...
          <h3 className="text-lg font-bold text-foreground mb-2">لا توجد دورات حالياً</h3>
          <p className="text-sm text-muted-foreground max-w-sm">
            {searchDebounced 
              ? 'لم نعثر على أي نتائج مطابقة لبحثك، جرب كلمات مفتاحية أخرى.' 
              : 'لا توجد دورات في هذا القسم حالياً، ستظهر الدورات هنا عند إرسالها للمراجعة.'}
          </p>
        </div>
      ) : (
              ...
              <div className="min-w-0 flex-1 space-y-2">
                ...
                <div className="flex flex-wrap items-center gap-4 text-xs text-muted-foreground">
                  ...
                  <span className="flex items-center gap-1.5">
                    ...
                    {new Date(c.submitted_for_review_at || c.created_at).toLocaleDateString('ar-EG', { year: 'numeric', month: 'short', day: 'numeric' })}
                  </span>
                </div>
              </div>
...
function StatusBadge({ status }: { status: string }) {
  const config: Record<string, { label: string; cls: string; Icon: any }> = {
    ...
    archived: { label: 'مؤرشفة', cls: 'bg-muted text-muted-foreground border border-border/50', Icon: GraduationCap },
  }

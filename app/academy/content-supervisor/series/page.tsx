...
import {
  ...
  User, ChevronLeft, Loader2, Layers, AlertCircle, Filter, ArrowLeft, Sparkles
} from 'lucide-react'
    ...
    <div className="space-y-8 animate-in fade-in duration-500 pb-8">
        ...
        <div className="flex items-start gap-4 bg-amber-500/10 border border-amber-500/30 rounded-2xl p-5 shadow-sm animate-in slide-in-from-top-4">
          ...
          <div>
            ...
            <p className="text-sm text-amber-800/80 dark:text-amber-500/80">
              يرجى تشغيل سكربت قاعدة البيانات <code className="font-mono bg-amber-500/20 px-1.5 py-0.5 rounded text-amber-900 dark:text-amber-300">scripts/030-content-review-workflow.sql</code> لتفعيل دورة مراجعة السلاسل.
            </p>
          </div>
        </div>
      )}
      ...
      <div className="bg-card border border-border/50 rounded-2xl shadow-sm p-4 space-y-4">
        <div className="relative">
          ...
          <input
            type="text"
            placeholder="ابحث بعنوان السلسلة أو اسم المعلم..."
            value={search}
      ...
      {loading ? (
        ...
        <div className="bg-card border border-border/50 rounded-2xl p-16 text-center flex flex-col items-center justify-center shadow-sm">
          ...
          <h3 className="text-lg font-bold text-foreground mb-2">لا توجد سلاسل حالياً</h3>
          <p className="text-sm text-muted-foreground max-w-sm">
            {searchDebounced 
              ? 'لم نعثر على أي نتائج مطابقة لبحثك، جرب كلمات مفتاحية أخرى.' 
              : 'لا توجد سلاسل في هذا القسم حالياً، ستظهر السلاسل هنا عند إرسالها للمراجعة.'}
          </p>
        </div>
      ) : (
              ...
              <div className="min-w-0 flex-1 space-y-2">
                <div>
                  ...
                  {s.subject && (
                    <p className="text-sm text-muted-foreground mt-0.5">
                      <span className="bg-muted px-2 py-0.5 rounded-md text-xs font-medium text-foreground/80">{s.subject}</span>
                    </p>
                  )}
                </div>
                ...
                <div className="flex flex-wrap items-center gap-4 text-xs text-muted-foreground">
                  {s.teacher_name && (
                    <span className="flex items-center gap-1.5 font-medium text-foreground/80">
                      <div className="w-5 h-5 rounded-full bg-primary/20 flex items-center justify-center overflow-hidden">
                        {s.teacher_avatar ? (
                          <img src={s.teacher_avatar} alt="" className="w-full h-full object-cover" />
                        ) : (
                  ...
                  <span className="flex items-center gap-1.5">
                    ...
                    {new Date(s.submitted_for_review_at || s.created_at).toLocaleDateString('ar-EG', { year: 'numeric', month: 'short', day: 'numeric' })}
                  </span>
                </div>
              </div>
...
function StatusBadge({ status }: { status: string }) {
  const config: Record<string, { label: string; cls: string; Icon: any }> = {
    ...
    published: { label: 'منشورة', cls: 'bg-emerald-500/10 text-emerald-700 border border-emerald-500/20 dark:text-emerald-400', Icon: CheckCircle },
    rejected: { label: 'مرفوضة', cls: 'bg-rose-500/10 text-rose-700 border border-rose-500/20 dark:text-rose-400', Icon: XCircle },
    draft: { label: 'مسودة', cls: 'bg-muted text-muted-foreground border border-border/50', Icon: Library },
  }

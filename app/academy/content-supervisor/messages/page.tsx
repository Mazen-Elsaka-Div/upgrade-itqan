...
import { Search, Send, User, Loader2, ArrowRight, MessageSquare, Sparkles } from 'lucide-react'
...
export default function ContentSupervisorMessagesPage() {
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-100px)] animate-in fade-in duration-500" dir="rtl">
      {/* Header */}
      <div className="shrink-0 mb-6">
        ...
        <p className="text-muted-foreground text-sm flex items-center gap-2 mt-2">
          ...
          تواصل مباشر مع المعلمين والمدراء
        </p>
      </div>

      <div className="flex bg-card rounded-3xl border border-border/50 flex-1 overflow-hidden min-h-0 shadow-sm">
        {/* Sidebar */}
        <div className={`w-full md:w-[340px] border-l border-border/50 flex flex-col min-h-0 bg-muted/10 ${activeConv ? 'hidden md:flex' : 'flex'}`}>
          <div className="p-4 border-b border-border/50 bg-card">
            <div className="relative">
              ...
              <input
                ...
                className="w-full h-11 pr-10 pl-4 bg-background border border-border/50 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 transition-all shadow-sm placeholder:text-muted-foreground"
              />
            </div>
          </div>
          ...
          <div className="flex-1 overflow-y-auto p-3 space-y-1.5 scrollbar-hide">
            {loadingConv ? (
              ...
              <div className="flex flex-col items-center justify-center h-full text-center space-y-3 opacity-60">
                ...
                <p className="text-muted-foreground text-sm font-medium">لا توجد محادثات بعد</p>
              </div>
            ) : filtered.map(conv => (
              <button
                key={conv.id}
                onClick={() => setActiveConv(conv)}
                className={`w-full text-right p-3 rounded-2xl flex items-center gap-4 transition-all group border ${
              ...
              >
                ...
                <div className="flex-1 min-w-0 text-right">
                  ...
                  <p className={`text-xs truncate ${conv.unread_count > 0 ? 'text-foreground font-bold' : 'text-muted-foreground'}`}>
                    {conv.last_message || 'اضغط لبدء المحادثة'}
                  </p>
                </div>
              </button>
            ))}
        ...
        <div className={`flex-1 flex flex-col min-h-0 bg-background/50 ${!activeConv ? 'hidden md:flex' : 'flex'}`}>
          {!activeConv ? (
            <div className="flex-1 flex flex-col items-center justify-center text-muted-foreground gap-4">
              ...
              <p className="font-bold text-lg text-foreground/70">اختر محادثة للبدء</p>
              <p className="text-sm text-muted-foreground">التواصل السريع والفعال يبدأ من هنا</p>
            </div>
          ) : (
            <>
              {/* Chat Header */}
              <div className="px-6 py-4 border-b border-border/50 flex items-center gap-4 shrink-0 bg-card shadow-sm z-10">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-xs text-muted-foreground font-medium">محادثة نشطة</p>
                </div>
              </div>
                ...
                messages.map((msg, idx) => {
                  ...
                  return (
                    <Fragment key={msg.id}>
                      ...
                      <div className={`flex ${isMe ? 'justify-start' : 'justify-end'} animate-in slide-in-from-bottom-2`}>
                ...
                <form onSubmit={handleSend} className="flex gap-3">
                  ...
                  >
                    {sending
                      ...
                      : <Send className="w-5 h-5 rotate-180 group-hover:scale-110 transition-transform" />
                    }
                  </Button>
                </form>
              </div>

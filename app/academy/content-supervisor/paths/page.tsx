...
import {
  Search, Clock, CheckCircle, XCircle, Route as RouteIcon,
  User, ChevronLeft, Loader2, AlertCircle, Filter, ArrowLeft, Sparkles
} from 'lucide-react'
...
export default function ContentPathsListPage() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/academy/supervisor/paths?${params}`)
      .then(async r => {
        if (r.status === 503) {
    ...
    <div className="space-y-8 animate-in fade-in duration-500 pb-8">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div className="space-y-1">
          ...
          <p className="text-muted-foreground text-sm flex items-center gap-2">
            ...
            مراجعة مسارات التجويد والحفظ المرسلة من المقرئين قبل نشرها
          </p>
        </div>
      </div>
        ...
        <div className="flex items-start gap-4 bg-amber-500/10 border border-amber-500/30 rounded-2xl p-5 shadow-sm animate-in slide-in-from-top-4">
          ...
          <div>
            ...
            <p className="text-sm text-amber-800/80 dark:text-amber-500/80">
              يرجى تشغيل سكربت قاعدة البيانات <code className="font-mono bg-amber-500/20 px-1.5 py-0.5 rounded text-amber-900 dark:text-amber-300">scripts/030-content-review-workflow.sql</code> لتفعيل مراجعة المسارات.
            </p>
          </div>
        </div>
      )}
      ...
      <div className="bg-card border border-border/50 rounded-2xl shadow-sm p-4 space-y-4">
        <div className="relative">
          ...
          <input
            type="text"
            placeholder="ابحث بعنوان المسار أو اسم المقرئ..."
            value={search}
      ...
      {loading ? (
        ...
        <div className="bg-card border border-border/50 rounded-2xl p-16 text-center flex flex-col items-center justify-center shadow-sm">
          <div className="w-20 h-20 bg-muted/50 rounded-full flex items-center justify-center mb-6">
            <RouteIcon className="w-10 h-10 text-muted-foreground/50" />
          </div>
        ...
        <div className="grid grid-cols-1 gap-4">
          {paths.map(p => (
            <Link
              key={`${p.path_type}-${p.id}`}
              href={`/academy/content-supervisor/paths/${p.id}?type=${p.path_type}`}
            ...
            >
              ...
              <div className="min-w-0 flex-1 space-y-2">
                <div className="flex items-center gap-3">
                  ...
                  <span className={`shrink-0 text-[10px] font-bold px-2 py-0.5 rounded-full border ${p.path_type === 'memorization' ? 'bg-purple-500/10 text-purple-700 border-purple-500/20 dark:text-purple-400' : 'bg-emerald-500/10 text-emerald-700 border-emerald-500/20 dark:text-emerald-400'}`}>
                    {TYPE_LABEL[p.path_type] || p.path_type}
                  </span>
                </div>
...
function StatusBadge({ status }: { status: string }) {
  const config: Record<string, { label: string; cls: string; Icon: any }> = {
    ...
    draft: { label: 'مسودة', cls: 'bg-muted text-muted-foreground border border-border/50', Icon: RouteIcon },
  }

  ...
  return (
    <>
      ...
      <div className="-m-6 lg:-m-8 min-h-[calc(100vh-4rem)] flex flex-col bg-background/50 relative font-sans">
        ...
        {playing && (
          <div className="fixed bottom-0 inset-x-0 z-50 bg-card/95 backdrop-blur-md border-t-2 border-primary/20 shadow-2xl">
            <div className="max-w-3xl mx-auto px-4 py-3 flex items-center gap-3">
              ...
              {/* Now playing info */}
              <div className="flex-1 min-w-0">
                <div className="text-sm font-black text-foreground truncate" style={{ direction: 'rtl' }}>
                  {(() => {
                ...
                <div className="text-xs text-muted-foreground font-bold truncate flex items-center gap-1.5">
                  <Mic2 className="w-3 h-3 flex-shrink-0" />
                  {isAr ? currentReciter.nameAr : currentReciter.nameEn}
                </div>
              </div>

              {/* Prev / Next / Auto-continue / Close */}
              <div className="flex items-center gap-1 flex-shrink-0">
                <Button size="icon" variant="ghost" onClick={() => playAdjacent(-1)} className="rounded-xl h-9 w-9">
                ...
                <Button
                  ...
                  variant="ghost"
                  onClick={stopPlayback}
                  className="rounded-xl h-9 w-9 text-rose-500 hover:bg-rose-500/10"
                >
                  <X className="w-4 h-4" />
                </Button>
              </div>
            ...
            {audioError && (
              <div className="border-t border-rose-500/20 bg-rose-500/10 px-4 py-1.5 text-xs font-bold text-rose-600 dark:text-rose-400 text-center">
                {audioError}
              </div>
            )}
          </div>
        ...
        {/* Spacer so fixed player doesn't cover nav buttons */}

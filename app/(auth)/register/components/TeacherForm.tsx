  ...
  return (
    <div className="w-full">
      <div className="flex items-center mb-6">
        ...
        <h2 className="text-2xl font-bold text-card-foreground mr-2">{isAr ? 'تسجيل معلم جديد' : 'Register New Teacher'}</h2>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        ...
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="md:col-span-2">
              <label htmlFor="full_name" className="block text-sm font-medium text-card-foreground/80 mb-1">{isAr ? 'الاسم الثلاثي *' : 'Triple Full Name *'}</label>
            ...
            <div>
              <label htmlFor="teacher_email" className="block text-sm font-medium text-card-foreground/80 mb-1">{isAr ? 'البريد الإلكتروني *' : 'Email Address *'}</label>
              <div className="relative">
            ...
            <div>
              <label htmlFor="teacher_password" className="block text-sm font-medium text-card-foreground/80 mb-1">{isAr ? 'كلمة المرور *' : 'Password *'}</label>
              <div className="relative">
            ...
            <div>
              <label htmlFor="teacher_phone" className="block text-sm font-medium text-card-foreground/80 mb-1">{isAr ? 'رقم الجوال *' : 'Mobile Number *'}</label>
              <div className="relative">
            ...
            <div>
              <label htmlFor="teacher_gender" className="block text-sm font-medium text-card-foreground/80 mb-1">{isAr ? 'الجنس *' : 'Gender *'}</label>
              <div className="relative">
                <select id="teacher_gender" value={form.gender} onChange={(e) => updateField('gender', e.target.value)} className="w-full pr-4 pl-4 py-3 bg-background border border-border rounded-xl focus:ring-2 focus:ring-primary focus:border-primary transition-colors text-sm text-card-foreground appearance-none" required>
            ...
            <div>
              <label className="block text-sm font-medium text-card-foreground/80 mb-1">{isAr ? 'الجنسية *' : 'Nationality *'}</label>
              <Popover open={nationalityOpen} onOpenChange={setNationalityOpen}>
                <PopoverTrigger asChild>
                  <button type="button" className={cn("w-full pr-4 pl-4 py-3 bg-background border border-border rounded-xl focus:ring-2 focus:ring-primary focus:border-primary transition-colors text-sm flex items-center justify-between text-right", !form.nationality && "text-muted-foreground")}>
        ...
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label htmlFor="qualification" className="block text-sm font-medium text-card-foreground/80 mb-1">{isAr ? 'المؤهل العلمي' : 'Scientific Qualification'}</label>
            ...
            <div>
              <label htmlFor="years_exp" className="block text-sm font-medium text-card-foreground/80 mb-1">{isAr ? 'سنوات الخبرة' : 'Years of Experience'}</label>
            ...
            <div className="md:col-span-2">
              <label htmlFor="teaching_subjects" className="block text-sm font-medium text-card-foreground/80 mb-1">{isAr ? 'المواد التي تدرّسها' : 'Subjects You Teach'}</label>
              <textarea id="teaching_subjects" value={form.teaching_subjects} onChange={(e) => updateField('teaching_subjects', e.target.value)} placeholder={isAr ? "مثال: تجويد القرآن، العقيدة، الفقه، اللغة العربية..." : "e.g., Quran Tajweed, Creed, Fiqh, Arabic Language..."} rows={3} className="w-full px-4 py-3 bg-background border border-border rounded-xl focus:ring-2 focus:ring-primary focus:border-primary transition-colors text-sm text-card-foreground resize-none" />
            </div>
          </div>
        </div>

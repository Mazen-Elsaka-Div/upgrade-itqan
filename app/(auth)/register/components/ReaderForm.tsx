  ...
  return (
    <div className="w-full">
      <div className="flex items-center mb-6">
        ...
        <h2 className="text-2xl font-bold text-card-foreground mr-2">{t.readerRegister.title}</h2>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        ...
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="md:col-span-2">
              <label htmlFor="full_name" className="block text-sm font-medium text-card-foreground/80 mb-1">{t.readerRegister.fullNameTriple}</label>
              <div className="relative">
                <User className="absolute right-3 top-1/2 -translate-y-1/2 text-muted-foreground w-5 h-5" />
                <input id="full_name" type="text" value={form.full_name_triple} onChange={(e) => updateField('full_name_triple', e.target.value)} placeholder={t.readerRegister.fullNamePlaceholder} className="w-full pr-10 pl-4 py-3 bg-background border border-border rounded-xl focus:ring-2 focus:ring-primary focus:border-primary transition-colors text-sm text-card-foreground" required />
              </div>
            </div>
            <div>
              <label htmlFor="reader_email" className="block text-sm font-medium text-card-foreground/80 mb-1">{t.readerRegister.email}</label>
              <div className="relative">
            ...
            <div>
              <label htmlFor="reader_password" className="block text-sm font-medium text-card-foreground/80 mb-1">{t.readerRegister.password}</label>
              <div className="relative">
                <Lock className="absolute right-3 top-1/2 -translate-y-1/2 text-muted-foreground w-5 h-5" />
                <input id="reader_password" type={showPw ? 'text' : 'password'} value={form.password} onChange={(e) => updateField('password', e.target.value)} placeholder={t.auth.passwordPlaceholder} dir="ltr" className="w-full pr-10 pl-10 py-3 bg-background border border-border rounded-xl focus:ring-2 focus:ring-primary focus:border-primary transition-colors text-sm text-card-foreground" required minLength={6} />
            ...
            <div>
              <label htmlFor="reader_phone" className="block text-sm font-medium text-card-foreground/80 mb-1">{t.readerRegister.phone}</label>
              <div className="relative">
            ...
            <div>
              <label htmlFor="reader_gender" className="block text-sm font-medium text-card-foreground/80 mb-1">{t.readerRegister.gender}</label>
              <div className="relative">
                <ChevronDown className="absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground w-5 h-5 pointer-events-none" />
                <select id="reader_gender" value={form.gender} onChange={(e) => updateField('gender', e.target.value)} className="w-full pr-4 pl-10 py-3 bg-background border border-border rounded-xl focus:ring-2 focus:ring-primary focus:border-primary transition-colors text-sm text-card-foreground appearance-none" required>
            ...
            <div>
              <label htmlFor="reader_nationality" className="block text-sm font-medium text-card-foreground/80 mb-1">{t.readerRegister.nationality}</label>
              <div className="relative">
                <Popover open={nationalityOpen} onOpenChange={setNationalityOpen}>
                  <PopoverTrigger asChild>
                    <button type="button" className={cn("w-full pr-4 pl-10 py-3 bg-background border border-border rounded-xl focus:ring-2 focus:ring-primary focus:border-primary transition-colors text-sm flex items-center justify-between", t.locale === 'ar' ? "text-right" : "text-left", !form.nationality && "text-muted-foreground")}>
        ...
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
              <label htmlFor="qualification" className="block text-sm font-medium text-card-foreground/80 mb-1">{t.readerRegister.qualification}</label>
              <input id="qualification" type="text" value={form.qualification} onChange={(e) => updateField('qualification', e.target.value)} placeholder={t.readerRegister.qualificationPlaceholder} className="w-full px-4 py-3 bg-background border border-border rounded-xl focus:ring-2 focus:ring-primary focus:border-primary transition-colors text-sm text-card-foreground" />
            </div>
            <div>
              <label htmlFor="memorized_parts" className="block text-sm font-medium text-card-foreground/80 mb-1">{t.readerRegister.memorizedParts}</label>
            ...
            <div>
              <label htmlFor="years_exp" className="block text-sm font-medium text-card-foreground/80 mb-1">{t.readerRegister.yearsOfExperience}</label>
        ...
        <div>
          ...
          <div className="border-2 border-dashed border-border rounded-xl p-6 text-center hover:border-primary/40 transition-colors">
            ...
            <p className="text-sm text-card-foreground/80 mb-1">{t.readerRegister.uploadCert}</p>
            ...
            <label htmlFor="certificate_file" className="inline-block mt-3 px-4 py-2 bg-secondary/30 hover:bg-secondary/50 rounded-lg text-sm text-card-foreground cursor-pointer transition-colors">
              {t.readerRegister.chooseFile}
            </label>
          </div>
        </div>

        <button type="submit" disabled={loading} className="w-full bg-primary hover:bg-primary/90 text-primary-foreground font-bold py-3.5 px-4 rounded-xl shadow-lg hover:shadow-xl transition-all duration-300 flex items-center justify-center gap-2 disabled:opacity-60">
          {loading ? (

...
-- Run on the LIVE DB. Change the email below if needed.
-- Nothing here writes data.
-- =====================================================================

\set email '''sayedxiv@gmail.com'''
...
WHERE e.student_id = (SELECT id FROM users WHERE email = :email)
...
WHERE student_id = (SELECT id FROM users WHERE email = :email)

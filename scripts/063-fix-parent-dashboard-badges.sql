-- ============================================
-- 063: Fix Parent Dashboard Badges
-- Fixes relation "student_badges" does not exist in parent dashboard functions.
-- It replaces it with the correct "badges" table.
-- ============================================
...
BEGIN
  ...
  badges_count AS (
    ...
    FROM active_children ac
    LEFT JOIN badges pb ON pb.user_id = ac.child_id
    GROUP BY ac.child_id
  )
  SELECT COALESCE(
    jsonb_agg(
  ...
  LEFT JOIN badges_count b ON b.child_id = ac.child_id;
